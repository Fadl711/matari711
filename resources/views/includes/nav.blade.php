    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            <div class="hidden lg:flex items-center gap-0.5 xl:gap-1.5">
                <a href="{{ route('home') }}"
                    class="px-2 xl:px-2 py-2 text-sm text-white hover:bg-primary-600 rounded-lg transition-colors duration-200 flex items-center gap-1 shrink-0">
                    <i class="fas fa-home hidden xl:inline-block text-xs"></i>
                    <span>الرئيسية</span>
                </a>
                ...
                @php
                    $allNavSections = \App\Models\Section::whereNull('parent_id')
                        ...
                        ->with('children')
                        ->get();
                    $navSections = $allNavSections;
                @endphp

                @foreach ($navSections as $navSection)
                    @if ($navSection->children->count() > 0)
                        <!-- قسم به أقسام فرعية -->
                        <div x-data="{ dropOpen: false }" @click.away="dropOpen = false" class="relative shrink-0">
                            <div class="flex items-center">
                                <a href="{{ route('posts.section', $navSection->id) }}"
                                    class="px-2 xl:px-2 py-2 text-sm text-white hover:bg-primary-600 rounded-r-lg transition-colors duration-200 flex items-center gap-1 shrink-0">
                                    <i class="fas {{ $navSection->icon }} hidden xl:inline-block text-xs"></i>
                                    <span>{{ $navSection->name }}</span>
                                </a>
                                <button @click.prevent="dropOpen = !dropOpen"
                                    class="px-1 py-2 text-white hover:bg-primary-600 rounded-l-lg transition-colors duration-200 shrink-0">
                            ...
                            <div x-show="dropOpen" x-transition:enter="transition ease-out duration-200"
                                ...
                                style="display: none;">
                                <div
                                    class="bg-white rounded-xl shadow-xl min-w-[200px] border border-cream-200 overflow-hidden flex flex-col">
                                    <a href="{{ route('posts.section', $navSection->id) }}"
                                        class="block px-4 py-3 text-brown-700 hover:bg-primary-50 hover:text-primary-600 transition-colors border-b border-cream-100 font-bold text-sm shrink-0">
                ...
                <a href="{{ route('about') }}"
                    class="px-2 xl:px-2 py-2 text-sm text-white hover:bg-primary-600 rounded-lg transition-colors duration-200 flex items-center gap-1 shrink-0 whitespace-nowrap">
                    <i class="fas fa-user-graduate hidden xl:inline-block text-xs"></i>
            ...
            <div class="flex items-center gap-2">
                    ...
                    @auth
                        @if (Auth::user()->usertype == 'admin' || Auth::user()->usertype == 'admin2')
                            <a href="{{ route('dashboard') }}"
                                class="bg-gold-500 text-primary-900 px-3 py-1.5 md:px-4 md:py-2 rounded-lg text-sm font-medium hover:bg-gold-400 transition-colors duration-200 flex items-center gap-1.5 shrink-0">
                ...
                <div class="lg:hidden flex items-center">
                    <button @click="open = !open"
                        class="text-white p-2 rounded-lg hover:bg-primary-600 transition-colors">
