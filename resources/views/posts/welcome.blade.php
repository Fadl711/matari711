...
<section class="islamic-gradient relative overflow-hidden">
    ...
    <div class="absolute inset-0 opacity-10">
        <div class="absolute inset-0" style="background-image: url('data:image/svg+xml,<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 80 80\"><path fill=\"%23ffffff\" d=\"M40 0L80 40L40 80L0 40z\" opacity=\"0.1\"/></svg>'); background-size: 40px 40px;"></div>
    </div>
    
    <div class="max-w-7xl mx-auto px-4 py-16 md:py-24 relative z-10">
        <div class="text-center">
            <!-- الأيقونة -->
            <div class="inline-flex items-center justify-center w-20 h-20 md:w-24 md:h-24 bg-gold-500/20 rounded-full mb-6">
    ...
    <div class="absolute bottom-0 left-0 right-0">
        <svg viewBox="0 0 1440 120" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M0 120L60 105C120 90 240 60 360 45C480 30 600 30 720 37.5C840 45 960 60 1080 67.5C1200 75 1320 75 1380 75L1440 75V120H1380C1320 120 1200 120 1080 120C960 120 840 120 720 120C600 120 480 120 360 120C240 120 120 120 60 120H0Z" class="fill-cream-100"/>
        </svg>
    </div>
</section>
            ...
            @foreach($sections as $section)
                <a href="{{ route('posts.section', $section->id) }}" 
                   class="group bg-white rounded-2xl p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 card-hover border border-cream-300">
                    <div class="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-primary-400 to-primary-600 rounded-2xl flex items-center justify-center transform group-hover:scale-110 transition-transform duration-300">
    ...
    <div class="max-w-7xl mx-auto px-4">
        @foreach($sections as $section)
            @if($section->posts->count() > 0)
                <!-- عنوان القسم -->
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 border-b-2 border-primary-500 pb-4">
                        ...
                        <article class="bg-white rounded-2xl overflow-hidden shadow-lg card-hover group">
                            ...
                            <div class="relative h-48 overflow-hidden">
                                ...
                                <div class="absolute top-4 right-4">
                                    <span class="bg-gold-500 text-primary-900 px-3 py-1 rounded-full text-sm font-medium">
                                        {{ $section->name }}
                                    </span>
                                </div>
                            </div>
